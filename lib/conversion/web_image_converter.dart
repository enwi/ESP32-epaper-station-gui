...
class WebImageConverter extends ImageConverter {
  @override
  Future<List<int>> convert(
    ...
    while (numRows-- > 0) {
      ...
      for (int column = 0; column < hdr.width; ++column, bytesIn += 3) {
        BigInt bestDist = BigInt.parse('0x7fffffffffffffff');
        int bestIdx = 0;
        int ditherFudge = 0;
        ...
        for (i = 0; i < hdr.numColors; i++) {
          BigInt dist = BigInt.zero;

          dist += BigInt.from((rgb[0] - clut[i][0] + ditherFudge) *
              (rgb[0] - clut[i][0] + ditherFudge) *
              4750);
          dist += BigInt.from((rgb[1] - clut[i][1] + ditherFudge) *
              (rgb[1] - clut[i][1] + ditherFudge) *
              47055);
          dist += BigInt.from((rgb[2] - clut[i][2] + ditherFudge) *
...
//Provides WebImageConverter
ImageConverter getImageConverter() => WebImageConverter();
